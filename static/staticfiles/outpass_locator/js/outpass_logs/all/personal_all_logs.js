...
var PersonalAdvanceFilterDatatable = function() {
    ...
    var staff_personal_outpass_logs_table = $('#staff_personal_outpass_logs').DataTable({
    responsive: true,
    processing: true,
    serverPaging: true,
    ...
    serverSorting: true,
    ajax: {
        url: $('#staff_personal_outpass_logs').data('url'),
...
$('#outpass_search_personal').on('click', function(e) {
    ...
    $.each(params, function(i, val) {
        ...
        staff_personal_outpass_logs_table.column(i).search(val ? val : '', false, false);
    });
    staff_personal_outpass_logs_table.table().draw();
});
...
$('#outpass_reset_personal').on('click', function(e) {
			...
			$('.datatable-input-personal').each(function() {
				$(this).val('');
				staff_personal_outpass_logs_table.column($(this).data('col-index')).search('', false, false);
			});
			staff_personal_outpass_logs_table.table().draw();
		});
...
$('#disabled-days-personal').on('keydown', function (e) {
    ...
    staff_personal_outpass_logs_table.column(4).search(outpass_date_from && outpass_date_to , true, true);
    }
    staff_personal_outpass_logs_table.table().draw();
